...
export default async function verifyRequest(request: Request, publicKey: string): Promise<boolean> {
	...
	const verified = await crypto.subtle.verify("NODE-ED25519", cryptoKey, signature, encoder.encode(timestamp + body));
	return verified;
}
